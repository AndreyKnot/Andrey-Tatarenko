...
void Menu::menu() const /** Виклик метода діалогового меню*/
{
	...
	while (stop != 0) /** Цикл, який працює поки користувач не захоче вийти*/
	{
		...
		case 3:
			...
			break;
		case 4:
			cout << "Индивидуальное задание:\nУзнать, кто из студентов имеет задолжности, и какой процент задолжности по программированию" << endl;

			try
			{
				list.Task(); /** Виклик методу виконання індивідуального завдання */
			}
			catch (const std::exception& ex)
			{
				cout << ex.what() << endl;
