...
void ArrayList::Task()
{
	task = new int[listSize];
	int max, numb;

	cout << "Номера наибольших значений массивов: " << endl;
	for (size_t i = 0; i < listSize; i++)
	{
		max = arrayList[i].array[1];
		numb = 1;
		for (size_t j = 2; j <= arrayList[i].array[0]; j++)
		{
			if (arrayList[i].array[j] > max)
			{
				max = arrayList[i].array[j];
				numb = j;
			}
		}
		task[i] = numb;
